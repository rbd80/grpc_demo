...
import (
	...
	"github.com/rbd80/grpc_demo/api"
...
// Person is used to implement PersonServer
type Person struct {
	savedPersons []*api.PersonRequest
}

// CreatePerson creates a new Person
func (p *Person) CreatePerson(c context.Context, input *api.PersonRequest) (*api.PersonResponse, error){
	p.savedPersons = append(p.savedPersons, input)
	return &api.PersonResponse{Id: input.Id, Success:true}, nil
}

// GetPerson returns all persons using filtter
func (p *Person) GetPersons(fltr *api.PersonFilter, stream api.Person_GetPersonsServer) error {
